...
/**
 * Comparer deux notifications. En commançant par le numéro de ligne, puis celui
 * de la colonne.
 *
...
const metalint = function (source, checkers) {
    ...
    for (const checker of checkers) {
        for (const name in checker.linters) {
            // Charger l'enrobage du linter et l'utiliser pour vérifier le code
            // source.
            const wrapper = require("./wrapper/" + name);
            results.push(wrapper(source, checker.linters[name], checker.level));
        }
    }
