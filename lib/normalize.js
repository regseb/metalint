...
/**
 * Normalise la propriété <code>"name"</code>.
 *
 * @param {*}      rotten La valeur de la proptiété <code>"name"</code>.
 ...
 */
const name = function (rotten, auto, root) {
    let standard;
    if (undefined === rotten) {
        ...
        throw new Error("property 'name' is incorrect type (only string is" +
                        " accepted).");
    }
    return standard;
};
...
const reporters = function (rottens, auto, root) {
    ...
    if (undefined === rottens) {
        const Name = name(undefined, "console", root);
        standards = [
            new Name(level(undefined, auto.level),
                     output(undefined, process.stdout, root),
                     options(undefined, {}))
        ];
    } else if (Array.isArray(rottens)) {
        standards = rottens.map(function (rotten) {
            const Name = name(rotten.name, "console", root);
            return new Name(level(rotten.level, auto.level),
                            output(rotten.output, process.stdout, root),
                            options(rotten.options, {}));
        });
    } else if ("object" === typeof rottens) {
        const Name = name(rottens.name, "console", root);
        standards = [
            new Name(level(rottens.level, auto.level),
