...
const formatter = function (rotten, auto, root) {
    ...
    if (undefined === rotten) {
        ...
        if (FORMATTERS.includes(rotten.toLowerCase() + ".js")) {
            standard = require("./formatter/" + rotten.toLowerCase() + ".js");
        } else if (rotten.startsWith(".")) {
            standard = require(path.join(root, rotten));
        } else {
            standard = require(rotten);
        }
    } else {
