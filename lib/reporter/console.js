...
/**
 * Ecrire une ligne du code source.
 *
 * @param {number}         line    Le numéro de la ligne.
 * @param {Array.<string>} content Les lignes du fichiers.
 * @param {boolean}        active  La drapeau pour indiquer si la notice est
                                   dans cette ligne.
 * @param {Object}         writer  Le flux où écrire les résultats.
 */
let printCodeSourceLine = function (line, content, active, writer) {
...
/**
 * Ecrire le code source proche du lieu où le problème a été trouvé.
 *
 ...
 * @param {Object}         writer    Le flux où écrire les résultats.
 */
let printCodeSource = function (locations, content, writer) {
    let characters = [];
    ...
    for (let i = 0; i < characters.length; ++i) {
        ...
        let columns = characters[i].columns;
        if (0 !== columns.length) {
