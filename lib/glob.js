 ...
 * @return {string} Les caractères protégés.
 */
 ...
 * @return {Object.<string, Object>} La marque pour la négation et l'expression
 *                                   rationnelle.
 */
const compile = function (pattern) {
    ...
    for (let i = 0; i < glob.length; ++i) {
        if ("/" === glob[i]) {
            if ("/**/" === glob.substr(i, 4)) {
                regexp += "/(.*/)?";
                i += 3;
            } else if ("/**" === glob.substr(i, 3)) {
                if (glob.length === i + 3) {
                    regexp += "/.*";
                    i += 2;
                } else {
                    throw new Error(pattern + ": ** non suivi d'un slash");
                }
            } else {
                regexp += "/";
            }
        } else if ("*" === glob[i]) {
            // Si c'est le dernier caractère ou qu'il n'est pas suivi par
            // étoile.
            ...
            // une étoile.
                ...
                throw new Error(pattern + ": ** non précédé d'un slash");
            }
        } else if ("?" === glob[i]) {
            regexp += "[^/]";
        } else if ("[" === glob[i]) {
            ...
            if (i === glob.length) {
                throw new Error(pattern + ": ] manquant");
            }
        } else {
            regexp += sanitize(glob[i]);
        }
    ...
    return { negate, "regexp": new RegExp(regexp) };
};
 ...
 *                                   répertoire.
 * @return {string} <code>"MATCHED"</code> si le fichier respecte au moins un
...
const walk = function (bases, patterns, root) {
    if (0 === bases.length) {
        return deep(".", patterns.map(compile), root);
    }

    const files = [];
    for (const base of bases) {
        files.push(...deep(base, patterns.map(compile), root));
    }
    return files;
};
